...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    next_h = np.tanh(np.dot(x, Wx) + np.dot(prev_h, Wh) + b) #RNN中ht和ht-1的关系式
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    #网络读取一小批的标注数据x，(样本数为N，每条标注的长度为T)，
    #并使用这批标注所对应图片的特征作为网络的初始隐藏状态h0，
    #通过前向传播过程，获得各个样本在每一步推进中产生的隐藏状态h，并存储反向传播所需变量。
    N, T, D = x.shape
    H = h0.shape[1]
    prev_h = h0
    h = np.zeros((N, T, H))
...
def word_embedding_backward(dout, cache):
    ...
    x, w_shape = cache
    dW = np.zeros(w_shape)
    np.add.at(dW, x, dout)#按照x的下标把dout+dW存入dW中，
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return dW
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    H = prev_h.shape[1]
    z = x.dot(Wx) + prev_h.dot(Wh) + b #把Z变成(N，4H)，方便取出ifog，下面看公式对应既可
    i = sigmoid(z[:, 0:H])
    f = sigmoid(z[:, H:2*H])
    o = sigmoid(z[:, 2*H:3*H])
    g = np.tanh(z[:, 3*H:4*H])
    ...
    cache = (i, f, o, g, next_c, prev_c, prev_h, Wx, Wh, x) #保存反向传播参数
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    (i, f, o, g, next_c, prev_c, prev_h, Wx, Wh, x) = cache#没怎么细看。。。记得sigmoid导数是sigmoid*（1-sigmoid）
