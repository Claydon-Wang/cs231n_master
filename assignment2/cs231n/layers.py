...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]                  #N = 32x32x3
    reshape_x = x.reshape(N,-1)     # reshape_x = (1,3072)
    out = np.dot(reshape_x,w) + b   #计算评分
...
def affine_backward(dout, cache):
    ...
    dx=np.reshape(dx,x.shape)           #dx=(Nx32x32x3)
    x_new=x.reshape(x.shape[0],-1)      #x_new=(DxN)
    dw=np.dot(x_new.T,dout)             #(D,N)×(N,M)=(D,M)
    db=np.sum(dout,axis=0)              #(1,M)=行相加x(N.M),不要keepdims，因为db是一维数组
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        sample_mean = np.mean(x,axis=0) #计算矩阵x每一列的平均值
        sample_var = np.var(x,axis=0) #计算矩阵x每一列的方差
        x_hat = (x-sample_mean)/(np.sqrt(sample_var+eps))  #normalize
        out = gamma * x_hat + beta  #套公式
        cache = (x, sample_mean, sample_var, x_hat, eps, gamma, beta) #储存反向传播参数
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    N = x.shape[0]
    dgamma = np.sum(dout * x_hat, axis=0)                     # 第5行公式(见笔记)
...
def batchnorm_backward_alt(dout, cache):
    ...
    N = dout.shape[0]
    gamma, x_hat, mean, var_eps, x = cache
    dgamma = np.sum(dout * x_hat, axis=0)
    dbeta = np.sum(dout, axis=0)
    dx_hat = dout * gamma#(N, D)   
    dx = var_eps**-0.5 / N * (N * dx_hat - np.sum(dx_hat, axis=0) - x_hat * np.sum(dx_hat * x_hat, axis=0))
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        mask = np.random.randn(*x.shape) < p  #生成一个相同shape的随机矩阵，其为均匀分布的随机样本[0,1),和p比较，得到一个随机真值表作为随机失活遮罩(mask)
        out = x * mask / p                    #使用inverted dropout
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_out = int(1 + (H + 2 * pad - HH) / stride)    #计算卷积后的高度
    W_out = int(1 + (W + 2 * pad - WW) / stride)    #计算卷积后的宽度
    out = np.zeros((N, F, H_out, W_out))            #定义输出矩阵
    x_i = np.pad(x.copy(), ((0, 0),(0, 0),(pad, pad),(pad, pad)), 'constant') #进行0填充
    for n in range(N):
      for f in range(F):
          for i in range(H_out):
              for j in range(W_out):
                out[n, f, i, j] = np.sum(x_i[n, :, stride*i:HH+stride*i, stride*j:WW+stride*j]*w[f]) + b[f] #卷积
...
def conv_backward_naive(dout, cache):
    ...
    dw = np.zeros_like(w)
    dx = np.zeros_like(x)
    x_i = np.pad(x.copy(), ((0, 0),(0, 0),(pad, pad),(pad, pad)), 'constant')
    #######################CNN反向传播过程日后理解##############################
    for n in range(N):
      dx_i = np.zeros_like(x_i[0])
      for f in range(F):
          for i in range(H_out):
              for j in range(W_out):
                dw[f] += dout[n, f, i, j] * x_i[n, :, stride*i:HH+stride*i, stride*j:WW+stride*j]
                dx_i[:, stride*i:HH+stride*i, stride*j:WW+stride*j] += dout[n, f, i, j] * w[f]
      dx[n] = dx_i[:, pad:pad+H, pad:pad+W]
...
def max_pool_forward_naive(x, pool_param):
    ...
    stride = pool_param['stride']            # 移动步长
    N, C, H, W = x.shape
    H_out = int(1 + (H - pool_height) / stride)   # 池化结果矩阵高度
    W_out = int(1 + (W - pool_width) / stride)    # 池化结果矩阵宽度
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
      for c in range(C):
        for i in range(H_out):
          for j in range(W_out):
            x_tmp = x[n, c, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            dx[n, c, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width][x_tmp == np.max(x_tmp)] += dout[n, c, i, j]
